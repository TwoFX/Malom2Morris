...
Public Class Sectors
    ...
    Public Shared Function getsectors() As Dictionary(Of id, Sector)
        ...
        Catch ex As Exception
            ...
            MsgBox("Exception in getsectors" & vbCrLf & ex.ToString)
            Return Nothing
        End Try
    End Function
End Class
...
Public Class PerfectPlayer
    ...
    Public Overrides Sub Quit()
        If Main IsNot Nothing Then Main.LblPerfEval.Text = ""
    ...
    Public Function GetSec(s As GameState) As Sector
        ...
        Catch ex As Exception
            If TypeOf ex Is KeyNotFoundException Then Throw
            MsgBox("Exception in GetSec" & vbCrLf & ex.ToString)
            Return Nothing
        End Try
    End Function
    ...
    Public Overrides Sub OppToMove(s As GameState)
        ...
        Try
            ...
            If Not s.KLE Then 'mert a korongleveteles allasok nem leteznek kulon
                Main.LblPerfEval.Text = "Ev: " & ToHumanReadableEval(Eval(s)) & ", NGM: " & NumGoodMoves(s)
            Else
                Main.LblPerfEval.Text = "Ev: KLE (last: " & Main.LblPerfEval.Text & ")"
            End If
        Catch ex As KeyNotFoundException
            Main.LblPerfEval.Text = "No secfile"
        Catch ex As Exception
            MsgBox("Exception in OppToMove" & vbCrLf & ex.ToString)
        End Try
    End Sub
    ...
    Public Shared Function ToHumanReadableEval(e As gui_eval_elem2) As String
        Try
            Return e.ToString()
        Catch ex As Exception
            MsgBox("Exception in ToHumanReadableEval" & vbCrLf & ex.ToString)
            Return Nothing
        End Try
    End Function
    ...
    Public Function MoveValue(ByVal s As GameState, ByVal m As Move) As gui_eval_elem2
        ...
        Catch ex As Exception
            If TypeOf ex Is KeyNotFoundException Then Throw
            MsgBox("Exception in MoveValue" & vbCrLf & ex.ToString)
            Return Nothing
        End Try
    End Function
    ...
    Structure MoveValuePair
        ...
        Dim val As Double
    End Structure
    Const WRGMInf As Double = 2 'todo: jo ez?
...
cutoff:
    ...
    Private Function Eval(ByVal s As GameState) As gui_eval_elem2
        ...
        Catch ex As Exception
            If TypeOf ex Is KeyNotFoundException Then Throw
            MsgBox("Exception in Eval" & vbCrLf & ex.ToString)
        End Try
    End Function
